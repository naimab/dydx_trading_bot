...
# Calc Half Life
# pythonforfinance mean reversion part 2

def calculate_half_life(spread):
    if len(spread) < 2:
        return np.nan  # Return NaN or appropriate value if not enough data
...
def calculate_cointegration(series_1, series_2):
    ...
    return coint_flag, hedge_ratio, half_life, intercept
...
def store_cointegration_results(df_market_prices):
    ...
    for index, base_market in enumerate(markets[:-1]):
        ...
        for quote_market in markets[index + 1:]:
            ...
            if not series_1 or not series_2:
                continue
            # check cointegration
            coint_flag, hedge_ratio, half_life, intercept = calculate_cointegration(series_1, series_2)
    ...
    if criteria_met_pairs:
        ...
        print("No cointegrated pairs meet the criteria.")
        return "none"